...
void __afl_parse_argv(int* argc, char ***argv) {
  if (*argc < 2) {
    fprintf(stderr, "CarpetFuzz: At least 2 arguments are needed.\n");
    return;
  }

  FILE *raw = fopen((*argv)[1], "rb");
  if (raw == NULL) {
    fprintf(stderr, "CarpetFuzz: Can not open file argv[1]\n");
    return;
  }
  ...
  fprintf(stderr, "CarpetFuzz: Your argc: %d\n", new_argc);
  for (int i = 0; i < new_argc; i++) {
    fprintf(stderr, "CarpetFuzz: Your argv %d: #%s#\n", i, new_argv[i]);
  }
